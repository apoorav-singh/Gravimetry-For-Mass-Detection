...
print(
    "Define the observation plane in terms of z for example: 20 and bottom of the prism -10000"
)
        ...
        for i_xx in [0, 1]:
            for i_yy in [0, 1]:
                for i_zz in [0, 1]:
                    ...
                    foo_gzz1 = (z - z_pos[i_zz]) * np.arctan(
                        (xx - x_pos[i_xx])
                        * (yy - y_pos[i_yy])
                        / (r * (z - z_pos[i_zz]))
                    )
                    ...
                    if (
                        np.isnan(foo_gzz1).any()
                        or np.isnan(foo_gzz2).any()
                        or np.isnan(foo_gzz3).any()
                    ):
                        print(
                            "i_x = {} i_y = {} i_xx = {} i_yy = {}".format(
                                i_x, i_y, i_xx, i_yy
                            )
                        )
                        # print(foo_gzz1)
